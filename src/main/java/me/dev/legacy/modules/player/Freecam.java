...
public class Freecam extends Module {
    ...
    public Setting speed = this.register(new Setting("Speed", 0.5D, 0.1D, 5.0D));
    public Setting packet = this.register(new Setting("Cancel Packets", true));
    private double posX;
    private double posY;
    private double posZ;
    private float pitch;
    ...
    public void onEnable() {
        if (mc.field_71439_g != null) {
            ...
            mc.field_71439_g.field_71075_bZ.func_75092_a((float) (((Double) this.speed.getValue()).doubleValue() / 100.0D));
    ...
    public void onPacketSend(PacketEvent event) {
        if ((event.getPacket() instanceof CPacketPlayer || event.getPacket() instanceof CPacketInput) && ((Boolean) this.packet.getValue()).booleanValue()) {
            event.setCanceled(true);
        }
