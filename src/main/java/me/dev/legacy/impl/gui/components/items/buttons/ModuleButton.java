...
public class ModuleButton extends Button {
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        if (!this.items.isEmpty()) {
            if (((Boolean) ClickGui.getInstance().gears.getValue()).booleanValue()) {
                mc.func_110434_K().func_110577_a(this.logo);
                drawCompleteImage(this.x - 1.5F + (float) this.width - 7.4F + 16.0F, this.y - 2.2F - (float) LegacyGui.getClickGui().getTextOffset() - 2.0F, 12, 12);
            }
